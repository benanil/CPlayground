...
{
    s32 densitySize = mesher->chunkSize + 3;
...
static int3 tMesherFaceToLocalI(tTransitionDirection direction, s32 chunkSize, s32 x, s32 y, s32 z)
{
    switch (direction)
    {
        case tTransitionDirection_XMin: return (int3){ z, x, y };
        case tTransitionDirection_XMax: return (int3){ chunkSize - z, y, x };
        case tTransitionDirection_YMin: return (int3){ y, z, x };
        case tTransitionDirection_YMax: return (int3){ x, chunkSize - z, y };
        case tTransitionDirection_ZMin: return (int3){ x, y, z };
        case tTransitionDirection_ZMax: return (int3){ y, x, chunkSize - z };
        default: return (int3){ x, y, z };
    }
}

static float3 tMesherFaceToLocalF(tTransitionDirection direction, s32 chunkSize, f32 x, f32 y, f32 z)
{
    switch (direction)
    {
        case tTransitionDirection_XMin: return (float3){ z, x, y };
        case tTransitionDirection_XMax: return (float3){ (f32)chunkSize - z, y, x };
        case tTransitionDirection_YMin: return (float3){ y, z, x };
        case tTransitionDirection_YMax: return (float3){ x, (f32)chunkSize - z, y };
        case tTransitionDirection_ZMin: return (float3){ x, y, z };
        case tTransitionDirection_ZMax: return (float3){ y, x, (f32)chunkSize - z };
...
static s32 tMesherBoundaryMaskFromPoint(s32 chunkSize, s32 x, s32 y, s32 z)
{
    s32 mask = 0;
    if (x == 0) mask |= 1;
    if (y == 0) mask |= 2;
    if (z == 0) mask |= 4;
    if (x == chunkSize) mask |= 8;
    if (y == chunkSize) mask |= 16;
    if (z == chunkSize) mask |= 32;
    return mask;
}

static s32 tMesherBoundaryMaskFromEdge(s32 chunkSize, int3 a, int3 b)
{
    s32 mask = 0;
    if (a.x == 0 || b.x == 0) mask |= 1;
    if (a.y == 0 || b.y == 0) mask |= 2;
    if (a.z == 0 || b.z == 0) mask |= 4;
    if (a.x == chunkSize || b.x == chunkSize) mask |= 8;
    if (a.y == chunkSize || b.y == chunkSize) mask |= 16;
    if (a.z == chunkSize || b.z == chunkSize) mask |= 32;
    return mask;
}

static float3 tMesherSecondaryPosition(float3 vertex, float3 normal, s32 chunkSize, s32 boundaryMask)
{
    float3 delta = F3Zero();
    if ((boundaryMask & 1) && vertex.x < 1.0f)
        delta.x = 1.0f - vertex.x;
    else if ((boundaryMask & 8) && vertex.x > (f32)(chunkSize - 1))
        delta.x = (f32)(chunkSize - 1) - vertex.x;

    if ((boundaryMask & 2) && vertex.y < 1.0f)
        delta.y = 1.0f - vertex.y;
    else if ((boundaryMask & 16) && vertex.y > (f32)(chunkSize - 1))
        delta.y = (f32)(chunkSize - 1) - vertex.y;

    if ((boundaryMask & 4) && vertex.z < 1.0f)
        delta.z = 1.0f - vertex.z;
    else if ((boundaryMask & 32) && vertex.z > (f32)(chunkSize - 1))
        delta.z = (f32)(chunkSize - 1) - vertex.z;
...
static bool tMesherEmitVertex(tMeshData* meshData, float3 vertex, float3 normal,
                              s32 chunkSize, s32 lodScale, s32 boundaryMask, u32 vertexIndex)
{
    if (boundaryMask > 0)
    {
...
static bool tMesherRegular(tTransvoxelMesher* mesher)
{
    ...
    tMeshData* meshData = tMeshDataContainerGet(mesher->meshData, tMeshDataSlot_Main);
    if (!meshData)
        return false;
        ...
        for (s32 z = 0; z < chunkSize; z++)
        {
            for (s32 x = 0; x < chunkSize; x++)
            {
                ...
                for (u32 i = 0; i < cellVertCount; i++)
                {
                    ...
                    if (vertexCacheable)
                        vertexIndex = selectedCache[cachePosX * chunkSize * 4 + cachePosZ * 4 + cacheIdx];
                    ...
                    if (!vertexCacheable || vertexIndex == -1)
                    {
                        ...
                        if (cacheIdx == 0)
                        {
                            ...
                            if (mesher->lod > 0)
                                boundaryMask = tMesherBoundaryMaskFromPoint(chunkSize, vertPosX, vertPosY, vertPosZ);

                            normal = tMesherNormalAtSample(mesher, vertPosX + padding, vertPosY + padding, vertPosZ + padding);
                            if (vertexCacheable)
                                selectedCache[cachePosX * chunkSize * 4 + cachePosZ * 4 + cacheIdx] = vertexIndex;
                        }
                        else
                        {
                            ...
                            if (mesher->lod > 0)
                                boundaryMask = tMesherBoundaryMaskFromEdge(chunkSize, vertLocalPos0, vertLocalPos1);
...
static void tMesherTransition(tTransvoxelMesher* mesher, tMeshDataSlot slot, tTransitionDirection direction)
{
    ...
    for (s32 y = 0; y < chunkSize; y++)
    {
        for (s32 x = 0; x < chunkSize; x++)
        {
            ...
            int3 pos2 = I3AddI(tMesherFaceToLocalI(direction, chunkSize, x + 1, y,     0), padding);
            int3 pos6 = I3AddI(tMesherFaceToLocalI(direction, chunkSize, x,     y + 1, 0), padding);
            int3 pos8 = I3AddI(tMesherFaceToLocalI(direction, chunkSize, x + 1, y + 1, 0), padding);
            ...
            float3 pos1 = tMesherWorldFromLocal(mesher, tMesherFaceToLocalF(direction, chunkSize, (f32)x + 0.5f, (f32)y,        0.0f));
            float3 pos3 = tMesherWorldFromLocal(mesher, tMesherFaceToLocalF(direction, chunkSize, (f32)x,        (f32)y + 0.5f, 0.0f));
            float3 pos4 = tMesherWorldFromLocal(mesher, tMesherFaceToLocalF(direction, chunkSize, (f32)x + 0.5f, (f32)y + 0.5f, 0.0f));
            float3 pos5 = tMesherWorldFromLocal(mesher, tMesherFaceToLocalF(direction, chunkSize, (f32)x + 1.0f, (f32)y + 0.5f, 0.0f));
            float3 pos7 = tMesherWorldFromLocal(mesher, tMesherFaceToLocalF(direction, chunkSize, (f32)x + 0.5f, (f32)y + 1.0f, 0.0f));
            ...
            if (caseCode == 0 || caseCode == 511)
                continue;
			currentCache[0 * chunkSize + x] = -1;
			currentCache[1 * chunkSize + x] = -1;
			currentCache[2 * chunkSize + x] = -1;
			currentCache[7 * chunkSize + x] = -1;
            ...
            for (u32 i = 0; i < cellVertCount; i++)
            {
                ...
                if ((cacheDir & 2u) > 0u)
                    selectedCache = previousCache;
                if (vertexCacheable)
                    vertexIndex = selectedCache[cacheIdx * chunkSize + cachePosX];
                ...
                if (!vertexCacheable || vertexIndex == -1)
                {
                    ...
                    if (density0 == 0.0f || density1 == 0.0f)
                    {
                        ...
                        vertex = tMesherFaceToLocalF(direction, chunkSize,
                            (f32)x + (f32)cornerOffset.x * 0.5f,
                            (f32)y + (f32)cornerOffset.y * 0.5f,
                            0.0f);

                        if (lowResFace)
                        {
                            int3 localPos = tMesherFaceToLocalI(direction, chunkSize, x + cornerOffset.x / 2, y + cornerOffset.y / 2, 0);
                            boundaryMask = tMesherBoundaryMaskFromPoint(chunkSize, localPos.x, localPos.y, localPos.z);
                        ...
                        if (cacheDir == 8u)
                            currentCache[cacheIdx * chunkSize + x] = vertexIndex;
                        else if (vertexCacheable)
                            selectedCache[cacheIdx * chunkSize + cachePosX] = vertexIndex;
                    }
                    else
                    {
                        int3 cornerOffset0 = tTransitionCornerOffset[cornerIdx0];
                        int3 cornerOffset1 = tTransitionCornerOffset[cornerIdx1];
                        float3 corner0 = tMesherFaceToLocalF(direction, chunkSize, (f32)x + (f32)cornerOffset0.x * 0.5f, (f32)y + (f32)cornerOffset0.y * 0.5f, 0.0f);
                        float3 corner1 = tMesherFaceToLocalF(direction, chunkSize, (f32)x + (f32)cornerOffset1.x * 0.5f, (f32)y + (f32)cornerOffset1.y * 0.5f, 0.0f);
                        ...
                        if (lowResFace)
                        {
                            int3 local0 = tMesherFaceToLocalI(direction, chunkSize, x + cornerOffset0.x / 2, y + cornerOffset0.y / 2, 0);
                            int3 local1 = tMesherFaceToLocalI(direction, chunkSize, x + cornerOffset1.x / 2, y + cornerOffset1.y / 2, 0);
                            boundaryMask = tMesherBoundaryMaskFromEdge(chunkSize, local0, local1);
                        ...
                        {
                            float3 local0 = tMesherFaceToLocalF(direction, chunkSize, (f32)x + (f32)cornerOffset0.x * 0.5f, (f32)y + (f32)cornerOffset0.y * 0.5f, 0.0f);
                            float3 local1 = tMesherFaceToLocalF(direction, chunkSize, (f32)x + (f32)cornerOffset1.x * 0.5f, (f32)y + (f32)cornerOffset1.y * 0.5f, 0.0f);
                           ...
                           s32 lod, s32 neighboursMask, tMeshDataContainer* meshData, void* userData)
{
    (void)neighboursMask;
    (void)userData;
    if (!generator || !densityData || !meshData || chunkSize <= 0 || lod < 0)
    {
