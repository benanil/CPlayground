...
#ifndef _INPUT_OUTPUT_INCLUDED
#define _INPUT_OUTPUT_INCLUDED
...
inline void AFileRead(void* dst, uint64_t size, AFile file, int alignment) {
    fread(dst, alignment, size, file.file);
}

inline void AFileWrite(const void* src, uint64_t size, AFile file, int alignment) { 
...
inline char* ReadAllFile(const char* fileName, char* buffer)
{
...
inline char* ReadAllText(const char* fileName, char* buffer, uint64_t* numCharacters, const char* startText)
{
    int startTextLen = 0;
    if (startText) 
...
// buffer is pre allocated memory if exist. otherwise null. 
// note: if you define it you are responsible of deleting the buffer
inline void ACopyFile(const char* source, const char* dst, char* buffer)
{
...
inline void RemoveFolder(const char* path, void* unused) 
{
